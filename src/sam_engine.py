...
class SAMEngine:
    ...
    def infer(self, image: np.ndarray, points: np.ndarray, label: str,
              prompt_type: str = "bb", sampling_mode: str = "hr"
              ) -> Optional[np.ndarray]:
        """
        ...
        try:
            ...
            if prompt_type in ('pt', 'both'):
                ...
                # else:
                #     point_prompts = [[
                #         [int(x * 0.32), int(y * 0.32)] for x, y in point_prompts[0]
                #     ]]  # fix for 1280x1920 resized images

            if prompt_type in ('bb', 'both'):
                box_prompt = [[list(poly.bounds)]]  # [x_min, y_min, x_max, y_max]
                # box_prompt = [[[
                #     int(coord*0.32) for coord in poly.bounds
                # ]]]  # fix for 1280x1920 resized images
    ...
    def medial_axis_sampling(self, points: np.ndarray, hull_vertices: np.ndarray,
            offset: float = 0.2) -> List[List[List[float]]]:
        ...
        sampled_points = [[center_point.astype(int).tolist(),
            left_point.astype(int).tolist(), right_point.astype(int).tolist()]]

        return sampled_points
    
    def curvature_spline_sampling(self, points: np.ndarray, image_shape: tuple,
            num_samples: int = 5) -> List[List[List[float]]]:
